...
export const BookingProvider = ({ children }) => {
  ...
  const [counts, setCounts] = useState({ students: 0, counsellors: 0 });
  ...
  const addBooking = async (problem, date) => {
    try {
      const response = await api.post("/booking/create", { problem, date });
  ...
  const updateBookingStatus = async (bookingId, updateData) => {
    try {
      const response = await api.put(`/booking/update/${bookingId}`, updateData);
      setBookings((prev) =>
